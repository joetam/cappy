...
Pull the open menu popover away from the menu bar to turn it into the floating widget. Once it crosses the release threshold, macOS morphs it into the widget and provides subtle trackpad feedback. You can also use the screen button in the popover or right-click the menu-bar icon. It stays above ordinary windows and follows you across Spaces; drag it to move it. Cappy remembers its position and whether it was visible across launches.
