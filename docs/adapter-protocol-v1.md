...
- `primeQuota`: supported only by the built-in Codex adapter. It sends one adapter-owned, tool-free no-op message to
  start a newly refreshed weekly Codex quota window. The prompt must be a constant controlled by the adapter, never
  caller-supplied text, and the operation must not persist a conversation or permit tools. Other adapters reject it.
